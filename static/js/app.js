const app = {
    currentData: null,
    currentProjectId: null, // Track active project ID for updates
    isLoginMode: true,
    currentUser: null,
    ...
    loadProject: async (id) => {
        try {
            ...
            app.currentProjectId = id; // Store ID
    ...
    generatePlan: async () => {
        ...
        try {
            ...
            app.currentData = data.project_data || data; // Handle potential wrapped response
            app.currentProjectId = data.project_id || null; // Capture ID if returned
    ...
    renderResults: (data) => {
        ...
        if (data.risk_assessment) {
            ...
            const msg = data.risk_assessment.message || "No specific risks detected.";
            const mit = data.risk_assessment.mitigation || "None.";
            document.getElementById('riskMessage').innerText = msg;
            document.getElementById('riskMitigation').innerText = `>> Mitigation: ${mit}`;
        }
        ...
        // --- DATE LOGIC FIX ---
        let runningDate = new Date(); // Start today
        // formatter
        const formatDate = (d) => d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });

        data.phases.forEach((phase, index) => {
            // Parse duration (simple heuristic)
            let days = 1;
            const dStr = phase.duration.toLowerCase();
            if (dStr.includes('week')) days = parseInt(dStr) * 7 || 7;
            else if (dStr.includes('day')) days = parseInt(dStr) || 1;
            else if (dStr.includes('month')) days = parseInt(dStr) * 30 || 30;
            ...
            const completedCount = phase.tasks ? phase.tasks.filter(t => t.status === "Completed").length : 0;
            const totalCount = phase.tasks ? phase.tasks.length : 0;
            const progressColor = completedCount === totalCount && totalCount > 0 ? "text-green-400" : "text-text-muted";
            ...
            card.innerHTML = `
                ...
                <div class="flex items-start justify-between mb-2">
                    <div class="flex items-center gap-3">
                         <div class="flex flex-col items-center justify-center bg-surface-dark border border-gray-700 rounded px-2 py-1 min-w-[50px]">
                            <span class="text-[10px] uppercase text-gray-400 font-bold">Day</span>
                            <span class="text-sm font-bold text-white">${startDate.getDate()}</span>
                         </div>
                         <div>
                            <h3 class="text-base font-bold text-text-main group-hover:text-brand-purple transition-colors">${phase.name}</h3>
                            <span class="text-xs text-text-muted">${dateLabel} (${phase.duration})</span>
                         </div>
                    </div>
                ...
                <p class="text-sm text-text-muted leading-relaxed mb-4 ml-[62px]">${phase.description || ''}</p>
                
                <div class="flex items-center gap-4 border-t border-border-light pt-3 ml-[62px]">
                    <div class="flex items-center gap-1 text-xs ${progressColor}">
                        <span class="material-symbols-outlined text-[16px]">check_circle</span>
                        <span>${completedCount}/${totalCount} Tasks</span>
                    </div>
                    ${phase.ai_insight ? `<div class="flex items-center gap-1 text-xs text-accent-purple"><span class="material-symbols-outlined text-[16px]">lightbulb</span><span>AI Insight</span></div>` : ''}
                </div>
            `;
    ...
    openModal: (phaseIndex) => {
        ...
        phase.tasks.forEach((task, i) => {
            ...
            const dependencyHtml = task.dependencies && task.dependencies !== "None"
                ? `<div class="mt-1 text-[10px] text-orange-400 bg-orange-400/10 px-2 py-0.5 rounded inline-block w-fit">Warning: Needs ${task.dependencies}</div>`
                : '';

            taskEl.innerHTML = `
                <div class="relative flex items-center justify-center shrink-0">
                    <input type="checkbox" 
                        class="task-checkbox peer h-5 w-5 rounded border-slate-600 bg-transparent text-primary focus:ring-0 focus:ring-offset-0 transition-all checked:bg-primary checked:border-primary cursor-pointer" 
                        ${isCompleted ? "checked" : ""}
                        onchange="app.toggleTask(${phaseIndex}, ${i}, this.checked)"
                    />
                </div>
    ...
    toggleTask: async (phaseIndex, taskIndex, completed) => {
        // Optimistic UI Update
        app.currentData.phases[phaseIndex].tasks[taskIndex].status = completed ? "Completed" : "Pending";

        // Re-render project background views if needed or just sync state
        // app.renderResults(app.currentData); // Optional: might be too heavy?

        if (!app.currentProjectId) return; // Can't save if not persisted

        try {
            await fetch(`/project/${app.currentProjectId}/toggle_task`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ phase_index: phaseIndex, task_index: taskIndex, completed })
            });
            // Silent success or small toast?
            // app.showNotification("Saved", "neutral"); 
        } catch (e) {
            console.error("Failed to save task", e);
            app.showNotification("Failed to save progress", "error");
        }
    },
    ...
    checkAllTasks: () => {
        ...
        checkboxes.forEach(cb => {
            if (cb.checked !== !allChecked) cb.click(); // Trigger trigger handler
        });
